...
export default function InstitutionalHome() {
  ...
  async function createStructure() {
    ...
    try {
      const res = await fetch("/api/institutionnel/access", {
        method: "POST",
      ...
      const json = await res.json();
      if (!res.ok || !json?.id) {
        alert(json?.error || "Création de la structure impossible.");
        return;
      }
      // La structure ouvre directement l'environnement institutionnel complet.
      window.location.href = `/institutionnel/${json.id}`;
    } catch (error) {
      console.error("Création institutionnelle :", error);
      alert("Création impossible. Vérifie ta connexion puis réessaie.");
    } finally {
  ...
  return (
    <main className="institution-page">
      ...
      <section className="hero">
        ...
        {access.canCreate && <button className="create-structure-btn" onClick={() => setCreating((value) => !value)}>{creating ? "✕ Fermer" : "+ Créer une structure"}</button>}
      </section>
      ...
      {creating && (
        <section className="create-card">
          ...
          <div className="create-actions"><button type="button" className="secondary" onClick={() => setCreating(false)} disabled={creatingStructure}>Annuler</button><button type="button" className="primary" onClick={createStructure} disabled={creatingStructure}>{creatingStructure ? "Enregistrement…" : "Enregistrer et créer l’institution"}</button></div>
        </section>
      )}
