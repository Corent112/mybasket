...
type Structure = { id:string; name:string; short_name:string|null; city:string|null; email:string|null; season_label:string|null };
...
export default function InstitutionalPresetDocuments({structureId}:{structureId:string}){
  ...
  async function load(){
    const[a,b]=await Promise.all([
      supabase.from("institutional_structures").select("id,name,short_name,city,email,season_label").eq("id",structureId).single(),
  ...
  return <div className="pd-root">
    ...
    {active&&structure&&<div className="pd-modal" role="dialog" aria-modal="true"><div className="pd-dialog"><div className="pd-toolbar"><button className="ghost" onClick={close}>← Fermer</button><input value={title} onChange={e=>setTitle(e.target.value)} aria-label="Titre du document"/><div className="pd-actions"><button className="ghost" onClick={printDocument}>🖨 Imprimer / PDF</button><button onClick={()=>void save()} disabled={busy}>{busy?"Enregistrement…":"💾 Enregistrer"}</button></div></div>{message&&<div className="pd-success">✓ {message}</div>}<div className="pd-paper"><header><div className="pd-brand">MYBASKET</div><div className="pd-inst"><strong>{structure.name}</strong><span>{structure.season_label||""}{structure.city?` · ${structure.city}`:""}</span></div></header><h1>{active.title}</h1>{active.sections.map(sec=><section key={sec.title}><h2>{sec.title}</h2><div className="pd-fields">{sec.fields.map(f=><label key={f.key} className={f.span===2?"wide":""}>{f.type!=="checkbox"&&<span>{f.label}</span>}{f.type==="textarea"?<textarea value={values[f.key]||""} onChange={e=>setField(f.key,e.target.value)}/>:f.type==="select"?<select value={values[f.key]||""} onChange={e=>setField(f.key,e.target.value)}>{(f.options||[]).map(o=><option key={o} value={o}>{o||"—"}</option>)}</select>:f.type==="checkbox"?<span className="pd-check"><input type="checkbox" checked={!!values[f.key]} onChange={e=>setField(f.key,e.target.checked)}/><span>{f.label}</span></span>:<input type={f.type||"text"} placeholder={f.placeholder} value={values[f.key]||""} onChange={e=>setField(f.key,e.target.value)}/>}</label>)}</div></section>)}</div></div></div>}
    <style jsx>{`
      .pd-root{display:grid;gap:18px}.pd-heading{display:flex;justify-content:space-between;align-items:end;margin-bottom:10px}.pd-heading div{display:grid;gap:2px}.pd-heading b{font-size:1rem;color:#321015}.pd-heading span{font-size:.78rem;color:#81736c}.pd-template-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.pd-template-grid article{border:1px solid #eadfd9;border-radius:13px;background:#fff;padding:14px;display:grid;grid-template-columns:auto 1fr auto;gap:12px;align-items:center}.pd-icon{width:44px;height:44px;border-radius:10px;background:#f7eee9;display:grid;place-items:center;font-size:1.35rem}.pd-template-grid h3{margin:0 0 3px;font-size:.95rem}.pd-template-grid p{margin:0;color:#786a63;font-size:.76rem;line-height:1.35}.pd-root button{border:0;border-radius:9px;background:#6b1a2c;color:white;padding:9px 12px;font-weight:800;cursor:pointer}.pd-empty{padding:16px;border:1px dashed #dccbc3;border-radius:10px;color:#82736d;text-align:center}.pd-saved-list{display:grid;gap:7px}.pd-saved-list article{border:1px solid #eadfd9;border-radius:10px;padding:10px 12px;display:flex;justify-content:space-between;gap:10px;align-items:center}.pd-saved-list article>div:first-child{display:grid}.pd-saved-list span{font-size:.72rem;color:#8c7c75}.pd-saved-list article>div:last-child{display:flex;gap:6px}.pd-saved-list .danger{background:#fff;color:#9b2737;border:1px solid #dfbec5}.pd-modal{position:fixed;inset:0;background:rgba(26,14,16,.62);z-index:9999;overflow:auto;padding:22px}.pd-dialog{max-width:1050px;margin:0 auto}.pd-toolbar{position:sticky;top:0;z-index:2;background:#fff;border-radius:12px;padding:10px;display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center;box-shadow:0 8px 30px rgba(0,0,0,.16)}.pd-toolbar input{border:1px solid #dbcac4;border-radius:8px;padding:9px 10px;font-weight:800;min-width:0}.pd-actions{display:flex;gap:7px}.pd-root .ghost{background:#fff;color:#6b1a2c;border:1px solid #d8bbc2}.pd-success{background:#e9f7ee;color:#22663c;border-radius:8px;padding:8px 12px;margin:8px 0;font-size:.8rem;font-weight:800}.pd-paper{position:relative;overflow:hidden;background:white;max-width:850px;margin:14px auto;padding:112px 40px 55px;box-shadow:0 14px 42px rgba(67,23,35,.18);min-height:1050px}.pd-paper:before{content:"";position:absolute;left:-8%;right:-8%;top:-85px;height:175px;background:#6b1a2c;border-radius:0 0 48% 52%/0 0 32% 36%;transform:rotate(-1deg)}.pd-paper:after{content:"";position:absolute;left:-5%;right:-5%;top:88px;height:6px;background:#d4a24c;border-radius:50%}.pd-paper header{position:absolute;z-index:1;left:40px;right:40px;top:28px;display:flex;justify-content:space-between;gap:20px;color:#fff}.pd-brand{font-weight:1000;color:#fff;letter-spacing:.08em}.pd-inst{display:grid;text-align:right}.pd-inst span{font-size:.72rem;color:#f3dfe3}.pd-paper h1{text-align:left;color:#4d101d;font-size:1.75rem;margin:18px 0 28px}.pd-paper section{position:relative;margin:25px 0;padding-left:16px}.pd-paper section:before{content:"";position:absolute;left:0;top:0;width:4px;height:32px;border-radius:8px;background:#d4a24c}.pd-paper h2{font-size:.88rem;text-transform:uppercase;letter-spacing:.05em;background:none;border:0;margin:0 0 12px;padding:0;color:#6b1a2c}.pd-fields{display:grid;grid-template-columns:1fr 1fr;padding:0;gap:11px 18px}.pd-fields label{display:grid;gap:4px;font-size:.74rem;font-weight:700;color:#5a4c46}.pd-fields label.wide{grid-column:1/-1}.pd-fields input,.pd-fields select,.pd-fields textarea{width:100%;border:0;border-bottom:1px solid #cdbfc3;padding:7px 2px;background:#fff;font:inherit;font-weight:500;box-sizing:border-box}.pd-fields textarea{border:1px solid #c8bbb5;min-height:70px;resize:vertical}.pd-check{display:flex;gap:9px;align-items:flex-start;font-weight:500;line-height:1.35}.pd-check input{width:18px;height:18px;flex:none}.pd-actions button:disabled{opacity:.55;cursor:default}@media(max-width:750px){.pd-template-grid{grid-template-columns:1fr}.pd-template-grid article{grid-template-columns:auto 1fr}.pd-template-grid article>button{grid-column:1/-1}.pd-modal{padding:8px}.pd-toolbar{grid-template-columns:1fr 1fr}.pd-toolbar input{grid-column:1/-1;grid-row:2}.pd-actions{justify-content:flex-end}.pd-paper{padding:22px 16px;min-height:0}.pd-fields{grid-template-columns:1fr}.pd-fields label.wide{grid-column:auto}.pd-saved-list article{align-items:flex-start;flex-direction:column}}
    `}</style>
  </div>
}

function renderPrintable(t:Template,s:Structure,title:string,v:Record<string,any>){
 const fieldHtml=(f:Field)=>{const val=v[f.key];if(f.type==="checkbox")return `<div class="check ${f.span===2?"wide":""}"><span>${val?"☑":"☐"}</span><div>${esc(f.label)}</div></div>`;return `<div class="field ${f.span===2?"wide":""}"><label>${esc(f.label)}</label><div class="value">${f.type==="textarea"?esc(val).replace(/\n/g,"<br>"):esc(val)||"&nbsp;"}</div></div>`};
 return `<!doctype html><html><head><meta charset="utf-8"><title>${esc(title)}</title><style>@page{size:A4;margin:0}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#2c2523;margin:0;padding:34mm 15mm 20mm;position:relative;-webkit-print-color-adjust:exact;print-color-adjust:exact}body:before{content:"";position:fixed;left:-10%;right:-10%;top:-28mm;height:58mm;background:#6b1a2c;border-radius:0 0 48% 52%/0 0 32% 36%;transform:rotate(-1deg);z-index:-1}body:after{content:"";position:fixed;left:-5%;right:-5%;top:29mm;height:2mm;background:#d4a24c;border-radius:50%;z-index:-1}.head{position:fixed;top:10mm;left:15mm;right:15mm;display:flex;justify-content:space-between;color:#fff}.brand{color:#fff;font-weight:900;letter-spacing:.06em}.inst{text-align:right}.inst small{display:block;color:#f3dfe3;margin-top:2px}h1{text-align:left;color:#4d101d;font-size:25px;letter-spacing:-.02em;margin:5mm 0 7mm}section{position:relative;margin:7mm 0;break-inside:avoid;padding-left:5mm}section:before{content:"";position:absolute;left:0;top:0;width:1mm;height:9mm;border-radius:2mm;background:#d4a24c}h2{font-size:12px;text-transform:uppercase;letter-spacing:.05em;color:#6b1a2c;margin:0 0 3mm}.grid{display:grid;grid-template-columns:1fr 1fr;gap:3mm 7mm}.field{min-height:13mm;border-bottom:1px solid #d4c6ca;padding:2mm 0}.wide{grid-column:1/-1}.field label{font-size:7px;text-transform:uppercase;letter-spacing:.07em;color:#8a777d;font-weight:bold;display:block;margin-bottom:2mm}.value{min-height:5mm;font-size:11px;padding:1mm 0}.check{display:flex;gap:2mm;font-size:10px;line-height:1.4;align-items:flex-start;background:#fbf7f8;padding:2.5mm;border-radius:2mm}.check span{font-size:15px}.footer{position:fixed;left:15mm;right:15mm;bottom:7mm;border-top:1px solid #eadfe2;padding-top:2mm;font-size:7px;color:#8a777d;text-align:center}</style></head><body><div class="head"><div class="brand">MYBASKET</div><div class="inst"><b>${esc(s.name)}</b><small>${esc(s.season_label||"")}${s.city?` · ${esc(s.city)}`:""}</small></div></div><h1>${esc(t.title)}</h1>${t.sections.map(sec=>`<section><h2>${esc(sec.title)}</h2><div class="grid">${sec.fields.map(fieldHtml).join("")}</div></section>`).join("")}<div class="footer">Document généré depuis MyBasket · ${esc(s.name)}</div></body></html>`
}
