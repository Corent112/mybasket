...
const blankEvent=(date:string):EventForm=>({event_date:date,start_time:"09:00",end_time:"10:00",title:"",event_type:"formation",event_domain:"training",location:"",intervenant:"",description:"",cohort_id:""});
...
export default function InstitutionalCalendarHub({structureId,onGoTraining,onGoPlayers}:{structureId:string;onGoTraining:()=>void;onGoPlayers:()=>void}){
 ...
 async function load(){
  ...
  const[a,b,c]=await Promise.all([
   sb.from("institutional_events").select("id,event_date,start_time,end_time,title,event_type,event_domain,location,intervenant,description,source_type,cohort_id").eq("structure_id",structureId).eq("archived",false).order("event_date").order("start_time"),
 ...
 function editEvent(e:EventRow){setEventForm({id:e.id,event_date:e.event_date,start_time:e.start_time?.slice(0,5)||"",end_time:e.end_time?.slice(0,5)||"",title:e.title,event_type:e.event_type,event_domain:e.event_domain||"training",location:e.location||"",intervenant:e.intervenant||"",description:e.description||"",cohort_id:e.cohort_id||""})}

 async function saveEvent(){
  if(!eventForm||!eventForm.title.trim())return alert("Donne un titre à l’événement.");
  const{data:{user}}=await sb.auth.getUser();if(!user)return;
  const payload={structure_id:structureId,event_date:eventForm.event_date,start_time:eventForm.start_time||null,end_time:eventForm.end_time||null,title:eventForm.title.trim(),event_type:eventForm.event_type,event_domain:eventForm.event_domain,location:eventForm.location.trim()||null,intervenant:eventForm.intervenant.trim()||null,description:eventForm.description.trim()||null,cohort_id:eventForm.event_domain==="training"?(eventForm.cohort_id||null):null,updated_at:new Date().toISOString()};
 ...
 return <div className="hub">
  ...
  {eventForm&&<div className="modal" onMouseDown={e=>e.target===e.currentTarget&&setEventForm(null)}><form onSubmit={e=>{e.preventDefault();void saveEvent()}}><div className="modalHead"><div><small>CALENDRIER INSTITUTION</small><h3>{eventForm.id?"Modifier l’événement":"Ajouter un événement"}</h3></div><button type="button" onClick={()=>setEventForm(null)}>×</button></div><div className="fields"><label>Date<input type="date" value={eventForm.event_date} onChange={e=>setEventForm({...eventForm,event_date:e.target.value})}/></label><label>Rattaché à<select value={eventForm.event_domain} onChange={e=>{const domain=e.target.value as "training"|"player";setEventForm({...eventForm,event_domain:domain,event_type:domain==="training"?"formation":"stage",cohort_id:domain==="training"?eventForm.cohort_id:""})}}><option value="training">Formation des cadres</option><option value="player">Formation du joueur</option></select></label><label>Type<select value={eventForm.event_type} onChange={e=>setEventForm({...eventForm,event_type:e.target.value})}>{eventForm.event_domain==="training"?<><option value="formation">Formation</option><option value="meeting">Réunion</option><option value="other">Autre</option></>:<><option value="stage">Stage</option><option value="selection">Sélection</option><option value="detection">Détection</option><option value="meeting">Réunion</option><option value="other">Autre</option></>}</select></label><label>Début<input type="time" value={eventForm.start_time} onChange={e=>setEventForm({...eventForm,start_time:e.target.value})}/></label><label>Fin<input type="time" value={eventForm.end_time} onChange={e=>setEventForm({...eventForm,end_time:e.target.value})}/></label><label className="wide">Titre<input autoFocus value={eventForm.title} onChange={e=>setEventForm({...eventForm,title:e.target.value})} placeholder="Ex. Journée 1 - BFA"/></label><label>Lieu <small style={{fontWeight:700,color:"#9a7d72"}}>(facultatif)</small><input value={eventForm.location} onChange={e=>setEventForm({...eventForm,location:e.target.value})}/></label><label>Intervenant<input value={eventForm.intervenant} onChange={e=>setEventForm({...eventForm,intervenant:e.target.value})}/></label>{eventForm.event_domain==="training"&&<label className="wide">Formation liée <small style={{fontWeight:700,color:"#9a7d72"}}>(facultatif)</small><select value={eventForm.cohort_id} onChange={e=>setEventForm({...eventForm,cohort_id:e.target.value})}><option value="">Toutes / événement général cadres</option>{cohorts.map(c=><option key={c.id} value={c.id}>{c.planning_title||c.name}</option>)}</select></label>}<label className="wide">Description / informations<textarea value={eventForm.description} onChange={e=>setEventForm({...eventForm,description:e.target.value})} placeholder="Objectif, consignes, informations utiles…"/></label></div>{!eventForm.location.trim()&&<div className="locationWarning">⚠ Tu peux enregistrer sans lieu. Un point d’attention restera actif dans le calendrier et le Dashboard jusqu’à ce qu’un lieu soit renseigné.</div>}<div className="modalActions"><button type="button" className="ghost" onClick={()=>setEventForm(null)}>Annuler</button><button type="submit" className="primary">💾 Enregistrer l’événement</button></div></form></div>}
