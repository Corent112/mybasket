...
export default function InstitutionalWorkspace({structureId}:{structureId:string}){
 const sb=useMemo(()=>createClient(),[]);const[structure,setStructure]=useState<Structure|null>(null);const[tab,setTabState]=useState<Tab>("Dashboard");const[msg,setMsg]=useState("");
 ...
 return <main className="page" style={{"--inst-primary":primary,"--inst-secondary":secondary} as CSSProperties}>
  <section className="hero"><div className="identity">{structure.logo_url&&<img src={structure.logo_url} alt=""/>}<div><p>{structure.structure_type.toUpperCase()}</p><h1>{structure.name}</h1><span>{structure.season_label||"Saison non définie"}{structure.city?` · ${structure.city}`:""}</span></div></div><div className="heroActions"><button onClick={()=>go("Joueurs")}>🏀 Joueurs</button><button onClick={()=>go("Formation des cadres")}>🎓 Formations</button></div></section>
