...
import InstitutionalReferralPoster from "@/components/institutionnel/InstitutionalReferralPoster";
...
export default function InstitutionalPlayerWorkflow({ structureId }: { structureId: string }) {
  ...
  const [structurePrimaryColor, setStructurePrimaryColor] = useState("#172b54");
  ...
  const [sexFilter, setSexFilter] = useState<"all" | "boys" | "girls">("all");
  ...
  const emptyPlayerForm = () => ({
    first_name: "", last_name: "", birthdate: "", club_name: "", category: "", sex: "",
    height_cm: "", position: "", jersey_number: "", license_number: "", nationality: "",
    email: "", phone: "", guardian1_name: "", guardian1_phone: "", guardian1_email: "",
    guardian2_name: "", guardian2_phone: "", guardian2_email: "",
  });
  const [createPlayerForm, setCreatePlayerForm] = useState(emptyPlayerForm);
  ...
  async function loadSupportingData() {
    const [e, pa, t, sh, st, ss, branding] = await Promise.all([
      ...
      sb.from("institutional_structures").select("document_primary_color").eq("id", structureId).maybeSingle(),
    ]);
    ...
    setStructurePrimaryColor(String(branding.data?.document_primary_color || "#172b54"));
  ...
  async function createPlayer() {
    if (!createPlayerForm.first_name.trim() || !createPlayerForm.last_name.trim()) {
      return alert("Renseigne au minimum le prénom et le nom du joueur.");
    }
    const j = await playerAction("create_player", { player: createPlayerForm });
    if (!j?.player?.id) return;
    setCreatePlayerForm(emptyPlayerForm());
  ...
  async function deletePlayerById(player: Player) {
    if (!confirm(`Supprimer définitivement la fiche de ${player.first_name} ${player.last_name} ?\n\nÀ utiliser pour une erreur ou un doublon.`)) return;
    const j = await playerAction("delete_player", { playerId: player.id });
    if (!j) return;
    setSelected((v) => v.filter((id) => id !== player.id));
    await loadPlayersAndReferrals();
  }
  ...
  const normalizedSex = (value?: string | null) => String(value || "").trim().toLowerCase();
  const isBoy = (p: Player) => ["m", "male", "masculin", "garçon", "garcon", "boy", "homme"].includes(normalizedSex(p.sex));
  const isGirl = (p: Player) => ["f", "female", "féminin", "feminin", "fille", "girl", "femme"].includes(normalizedSex(p.sex));
  const boysCount = validatedPlayers.filter(isBoy).length;
  const girlsCount = validatedPlayers.filter(isGirl).length;
  const filteredPlayers = basePool.filter((p) => {
    const haystack = `${p.first_name} ${p.last_name} ${p.club_name || ""} ${p.category || ""}`.toLowerCase();
    const sexOk = sexFilter === "all" || (sexFilter === "boys" ? isBoy(p) : isGirl(p));
    return sexOk && (!search.trim() || haystack.includes(search.trim().toLowerCase())) && (!clubFilter || p.club_name === clubFilter) && (!categoryFilter || p.category === categoryFilter);
  });
  ...
  if (openedPlayer) {
    return (
      <div className="wf playerMode">
        <InstitutionalPlayerSheet
          structureId={structureId}
          player={openedPlayer}
          referral={referrals.find((r) => r.id === safeProfile(openedPlayer).sourceReferralId) || null}
          busy={busy}
          onClose={() => setOpenedPlayerId(null)}
          onSave={(data) => savePlayer(data as PlayerEditData, false)}
          onArchive={archivePlayer}
          onRestore={() => restorePlayer(openedPlayer.id)}
          onDelete={deletePlayer}
          players={validatedPlayers}
          referralUrl={referralUrl}
          onOpenPlayer={(playerId) => setOpenedPlayerId(playerId)}
    ...
    <div className="wf">
      <div className="hero">
        <div>
          ...
          <h2>{tab === "base" ? "Base de joueurs" : tab === "referrals" ? "Signalements" : tab === "detections" ? "Détections & sélections" : "Passations"}</h2>
          <span>{tab === "base" ? "Tous les joueurs identifiés, détectés ou suivis par l'Institution." : "Le parcours joueur reste centralisé dans une seule fiche, sans doublon."}</span>
        </div>
        {tab === "base" && referralUrl ? <div className="heroReferralActions">
          <div className="heroReferral">
            <img src={`https://api.qrserver.com/v1/create-qr-code/?size=110x110&margin=0&data=${encodeURIComponent(referralUrl)}`} alt="QR code signalement joueur" />
            <div><b>Signaler un joueur</b><span>Scanne le QR code ou partage le lien</span><div><code>{referralUrl.replace(/^https?:\/\//, "")}</code><button onClick={copyReferralLink}>Copier</button></div></div>
          </div>
          <button className="posterBtn" style={{ backgroundColor: structurePrimaryColor }} onClick={()=>setShowReferralPoster(true)}>Générer une affiche</button>
        </div> : <div className="heroStats">
          <b>{validatedPlayers.length}<small>Joueurs</small></b>
          <b>{pendingReferrals.length}<small>À vérifier</small></b>
        </div>}
      </div>

      {showReferralPoster && referralUrl && <InstitutionalReferralPoster structureId={structureId} referralUrl={referralUrl} onClose={()=>setShowReferralPoster(false)} />}
      ...
      {tab === "base" && (
        <>
          <section className="baseIntro">
            <div className="genderTabs">
              <button className={sexFilter === "all" ? "on" : ""} onClick={() => setSexFilter("all")}>Tous <span>{validatedPlayers.length}</span></button>
              <button className={sexFilter === "boys" ? "on" : ""} onClick={() => setSexFilter("boys")}>Garçons <span>{boysCount}</span></button>
              <button className={sexFilter === "girls" ? "on" : ""} onClick={() => setSexFilter("girls")}>Filles <span>{girlsCount}</span></button>
            </div>
            <div className="baseActions"><span className="viewHint">Clique sur un joueur pour ouvrir sa fiche complète.</span><button type="button" onClick={() => setShowCreatePlayer(true)}>+ Ajouter un joueur</button></div>
          </section>

          <section className="toolbar cleanToolbar">
            <div className="searchBox">⌕<input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher un joueur, un club, une catégorie…" /></div>
            <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}><option value="">Catégorie</option>{categories.map((x) => <option key={x}>{x}</option>)}</select>
            <select value={clubFilter} onChange={(e) => setClubFilter(e.target.value)}><option value="">Club</option>{clubs.map((x) => <option key={x}>{x}</option>)}</select>
            <label className="archiveToggle"><input type="checkbox" checked={showArchived} onChange={(e) => setShowArchived(e.target.checked)} /> Archivés</label>
          </section>

          <section className="playersPanel">
            <div className="playerGrid modernGrid">
              {filteredPlayers.map((p) => {
                const archived = p.workflow_status === "archived" || p.archived;
                const profile = safeProfile(p);
                const age = p.birthdate ? Math.max(0, new Date().getFullYear() - Number(p.birthdate.slice(0,4))) : null;
                const position = p.position_primary || profile.position || "Poste —";
                return <article className={`playerCard modernCard ${archived ? "archived" : ""}`} key={p.id} onClick={() => setOpenedPlayerId(p.id)}>
                  <label className="selectPlayer floatingSelect" onClick={(e) => e.stopPropagation()} title="Sélectionner pour une détection ou une passation"><input type="checkbox" disabled={archived} checked={selected.includes(p.id)} onChange={() => toggle(p.id)} /><span /></label>
                  <div className="cardPhoto">{p.photo_url ? <img src={p.photo_url} alt="" /> : `${p.first_name?.[0] || "?"}${p.last_name?.[0] || ""}`}</div>
                  <div className="cardBody">
                    <div className="cardName"><b>{p.first_name} {p.last_name}</b><span className={`sexMark ${isGirl(p) ? "girl" : "boy"}`}>{isGirl(p) ? "♀" : "♂"}</span></div>
                    <span className="metaLine">{p.category || "Catégorie —"}{age ? ` · ${age} ans` : ""}</span>
                    <span className="clubLine">{p.club_name || "Club non renseigné"}</span>
                    <span className="positionLine">{position}</span>
                    <div className="cardFooter"><b>{p.height_cm ? `${p.height_cm} cm` : "Taille —"}</b>{archived ? <em className="archivePill">Archivé</em> : profile?.selections?.length ? <em className="selectedPill">● Sélectionné</em> : <span />}</div>
                  </div>
                  <button className="cardDelete" type="button" title="Supprimer cette fiche / doublon" onClick={(e) => { e.stopPropagation(); void deletePlayerById(p); }}>×</button>
      ...
      {showCreatePlayer && <div className="createPlayerBack" onMouseDown={(e) => { if (e.target === e.currentTarget) setShowCreatePlayer(false); }}>
        <section className="createPlayerModal" role="dialog" aria-modal="true" aria-label="Ajouter un joueur">
          <div className="createPlayerHead"><div><p>BASE JOUEURS</p><h3>Ajouter un joueur</h3><span>La validation crée directement sa fiche joueur complète.</span></div><button type="button" className="close" onClick={() => setShowCreatePlayer(false)}>×</button></div>
          <div className="createPlayerScroll">
            <div className="createSection"><b>Identité</b><div className="createGrid">
              <label>Prénom *<input value={createPlayerForm.first_name} onChange={(e) => setCreatePlayerForm(v => ({...v, first_name:e.target.value}))} /></label>
              <label>Nom *<input value={createPlayerForm.last_name} onChange={(e) => setCreatePlayerForm(v => ({...v, last_name:e.target.value}))} /></label>
              <label>Date de naissance<input type="date" value={createPlayerForm.birthdate} onChange={(e) => setCreatePlayerForm(v => ({...v, birthdate:e.target.value}))} /></label>
              <label>Sexe<select value={createPlayerForm.sex} onChange={(e) => setCreatePlayerForm(v => ({...v, sex:e.target.value}))}><option value="">—</option><option value="M">Garçon</option><option value="F">Fille</option></select></label>
              <label>Nationalité<input value={createPlayerForm.nationality} onChange={(e) => setCreatePlayerForm(v => ({...v, nationality:e.target.value}))} /></label>
            </div></div>
            <div className="createSection"><b>Basket</b><div className="createGrid">
              ...
              <label>N° licence<input value={createPlayerForm.license_number} onChange={(e) => setCreatePlayerForm(v => ({...v, license_number:e.target.value}))} /></label>
            </div></div>
            <div className="createSection"><b>Contact joueur</b><div className="createGrid">
              <label>Email<input type="email" value={createPlayerForm.email} onChange={(e) => setCreatePlayerForm(v => ({...v, email:e.target.value}))} /></label>
              <label>Téléphone<input value={createPlayerForm.phone} onChange={(e) => setCreatePlayerForm(v => ({...v, phone:e.target.value}))} /></label>
            </div></div>
            <div className="createSection"><b>Parents / responsables</b><div className="createGrid">
              <label>Responsable 1<input value={createPlayerForm.guardian1_name} onChange={(e) => setCreatePlayerForm(v => ({...v, guardian1_name:e.target.value}))} /></label>
              <label>Téléphone responsable 1<input value={createPlayerForm.guardian1_phone} onChange={(e) => setCreatePlayerForm(v => ({...v, guardian1_phone:e.target.value}))} /></label>
              <label>Email responsable 1<input type="email" value={createPlayerForm.guardian1_email} onChange={(e) => setCreatePlayerForm(v => ({...v, guardian1_email:e.target.value}))} /></label>
              <label>Responsable 2<input value={createPlayerForm.guardian2_name} onChange={(e) => setCreatePlayerForm(v => ({...v, guardian2_name:e.target.value}))} /></label>
              <label>Téléphone responsable 2<input value={createPlayerForm.guardian2_phone} onChange={(e) => setCreatePlayerForm(v => ({...v, guardian2_phone:e.target.value}))} /></label>
              <label>Email responsable 2<input type="email" value={createPlayerForm.guardian2_email} onChange={(e) => setCreatePlayerForm(v => ({...v, guardian2_email:e.target.value}))} /></label>
            </div></div>
          </div>
          <div className="createPlayerFoot"><button className="ghost" type="button" onClick={() => setShowCreatePlayer(false)}>Annuler</button><button type="button" disabled={busy === "create_player"} onClick={() => void createPlayer()}>{busy === "create_player" ? "Création…" : "Créer la fiche joueur"}</button></div>
        </section>
      </div>}
      <style jsx>{css}</style>
    </div>
