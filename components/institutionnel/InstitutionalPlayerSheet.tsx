...
export default function InstitutionalPlayerSheet({ structureId, player, referral, busy, onClose, onSave, onArchive, onRestore, onDelete, players = [], referralUrl = "", onOpenPlayer, onCopyReferralLink }: Props) {
  ...
  const playerRailRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    if (!rail) return;
    const current = Array.from(rail.querySelectorAll<HTMLElement>("[data-player-id]")).find((el) => el.dataset.playerId === String(player.id));
  ...
  const quickPlayers = players.filter((p) => !p.archived);
  const referralDisplay = referralUrl.replace(/^https?:\/\//, "");
  const qrSrc = referralUrl ? `https://api.qrserver.com/v1/create-qr-code/?size=110x110&margin=0&data=${encodeURIComponent(referralUrl)}` : "";
  const scrollRail = (direction: number) => playerRailRef.current?.scrollBy({ left: direction * 520, behavior: "smooth" });

  return <div className="playerPage">
    <section className="sheet" aria-label="Fiche joueur MyBasket">
      <section className="playerSwitcher">
        <div className="switcherHead">
          <div>
            <span>BASE DE JOUEURS</span>
            <h3>Passer rapidement d’un joueur à l’autre</h3>
          </div>
          {referralUrl && <div className="referralQuick">
            <img src={qrSrc} alt="QR code pour signaler un joueur" />
            <div>
              <b>Signaler un joueur</b>
              <span>Scanne le QR code ou partage le lien</span>
              <div className="referralQuickLink"><code>{referralDisplay}</code><button onClick={onCopyReferralLink}>Copier</button></div>
            </div>
          </div>}
        </div>
        <div className="playerRailWrap">
          <button className="railArrow" onClick={() => scrollRail(-1)} aria-label="Joueurs précédents">‹</button>
          <div className="playerRail" ref={playerRailRef}>
            {quickPlayers.map((p) => {
              const current = p.id === player.id;
              const pProfile = p.profile_data || {};
              const pInitials = `${p.first_name?.[0] || "?"}${p.last_name?.[0] || ""}`.toUpperCase();
              return <button key={p.id} data-player-id={p.id} className={`railPlayer ${current ? "current" : ""}`} onClick={() => !current && onOpenPlayer?.(p.id)}>
                <span className="railAvatar">{p.photo_url ? <img src={p.photo_url} alt="" /> : pInitials}</span>
                <span className="railName">{p.first_name} {p.last_name}</span>
                <small>{p.category || pProfile.category || "Catégorie —"}</small>
              </button>;
            })}
          </div>
          <button className="railArrow" onClick={() => scrollRail(1)} aria-label="Joueurs suivants">›</button>
        </div>
      </section>
