...
export default function InstitutionalDashboardConnected({structureId,go}:Props){
 ...
 return <div className="dash">{warnings.length>0&&<section className="warningBox"><div className="warningTitle"><span className="warningIcon">!</span><div><small>WARNING</small><h3>Points d’attention</h3><p>{warnings.length} problème{warnings.length>1?"s":""} à traiter</p></div></div><div className="warningList">{warnings.map(w=><button key={w.id} onClick={()=>go("Mon calendrier")}><span className="dangerDot">!</span><div><b>{w.title}</b><small>{new Date(`${w.event_date}T12:00:00`).toLocaleDateString("fr-FR",{day:"numeric",month:"long",year:"numeric"})} · {w.message}</small></div><strong>Corriger →</strong></button>)}</div></section>}<div className="kpis">{cards.map(([l,v,t])=><button key={l} onClick={()=>go(t)}><span>{l}</span><b>{v}</b><small>Ouvrir →</small></button>)}</div><section><h3>Centre de pilotage</h3><p>Une donnée saisie dans une fiche joueur ou un dossier de formation alimente les vues opérationnelles. Utilise les raccourcis ci-dessous pour travailler sans ressaisie.</p><div className="quick"><button onClick={()=>go("Joueurs")}>🏀 Suivre les joueurs</button><button onClick={()=>go("Formation des cadres")}>🎓 Gérer les inscrits</button><button onClick={()=>go("Mon calendrier")}>📅 Voir tout le planning</button><button onClick={()=>go("Documents")}>📄 Générer / retrouver les documents</button><button onClick={()=>go("Communication")}>✉️ Contacter les publics</button></div></section><style jsx>{`.dash{display:grid;gap:12px}.warningBox{border:1px solid #efb5bd!important;background:#fff7f8!important}.warningTitle{display:flex;gap:11px;align-items:flex-start}.warningIcon,.dangerDot{display:grid;place-items:center;border-radius:50%;background:#b4233b;color:#fff;font-weight:1000}.warningIcon{width:38px;height:38px;font-size:1.25rem}.warningTitle small{color:#b4233b;font-weight:1000;letter-spacing:.12em}.warningTitle h3{color:#8d172c!important;margin:1px 0!important}.warningTitle p{margin:0!important;color:#9b5965!important}.warningList{display:grid;gap:7px;margin-top:12px}.warningList button{width:100%;display:grid;grid-template-columns:26px minmax(0,1fr) auto;gap:9px;align-items:center;text-align:left;background:#fff!important;border:1px solid #f0cbd1!important;border-radius:10px!important;padding:10px!important;color:#5c2630!important}.dangerDot{width:24px;height:24px}.warningList b,.warningList small{display:block}.warningList small{color:#9b5965;margin-top:2px}.warningList strong{color:#b4233b;font-size:.72rem}.kpis{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.kpis button{display:grid;text-align:left;background:#fff!important;color:#2d2023!important;border:1px solid #eadfd8!important;border-radius:14px!important;padding:14px!important}.kpis span{font-size:.72rem;color:#7f7169}.kpis b{font-size:1.55rem;color:#6b1a2c;margin:4px 0}.kpis small{color:#b37a20;font-weight:900}.dash section{background:#fff;border:1px solid #eadfd8;border-radius:14px;padding:15px}.dash h3{margin:0;color:#4d1420}.dash p{color:#7f7169}.quick{display:grid;grid-template-columns:repeat(5,1fr);gap:7px}.quick button{min-height:58px}@media(max-width:900px){.kpis{grid-template-columns:1fr 1fr}.quick{grid-template-columns:1fr 1fr}}@media(max-width:550px){.kpis,.quick{grid-template-columns:1fr}}`}</style></div>
}
