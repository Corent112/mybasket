        ...
        {tab === 'workflow' ? (
          <div className="lcsBody">
            <div className="lcsProfileBar">
              ...
              {selectedProfile && <button className="deleteProfile" onClick={() => { if (window.confirm(`Supprimer « ${selectedProfile.name} » ?`)) { onDeleteProfile(selectedProfile.id); setProfileId(''); setProfileName(''); } }}>🗑</button>}
            </div>
          ...
          <div className="lcsBody">
            <div className="lcsInfo">Chaque bloc garde une présentation identique au codage. Renommer conserve la key stable : les anciennes actions restent reliées. Les blocs à logique statistique fixe peuvent être renommés/masqués mais pas recevoir de nouveau bouton arbitraire.</div>
                ...
                return (
                  <section className="lcsBlock" key={group.key}>
                    <div className="lcsBlockHead">
                      ...
                      {group.allowAdd !== false && <button onClick={() => add(group.key)}>＋ Ajouter</button>}
                    </div>
