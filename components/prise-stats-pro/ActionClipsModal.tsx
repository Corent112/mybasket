        ...
        <div className="acm-body">
          ...
          {hasVideo && (() => {
            ...
            return <div className="acm-trim-editor">
              ...
              <div className="acm-seek-controls">
                <button onClick={() => nudge(-seekStep)}>↶ −{seekStep}s</button>
                <label>Pas <select value={seekStep} onChange={(e)=>setSeekStep(Math.max(.1,Number(e.target.value)||5))}><option value={1}>1 s</option><option value={3}>3 s</option><option value={5}>5 s</option><option value={10}>10 s</option><option value={15}>15 s</option><option value={30}>30 s</option></select></label>
                <button onClick={() => nudge(seekStep)}>+{seekStep}s ↷</button>
              </div>
