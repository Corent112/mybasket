...
export default function TrainingManager({ institutionId }: { institutionId?: string }) {
  ...
  async function addCandidate() {
    ...
    setBusy(true);
    const response = await fetch("/api/training/candidates", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ cohortId, email: candidateEmail }) });
  ...
  async function setDocumentStatus(candidateId: string, requestId: string, status: string) {
    const response = await fetch("/api/training/candidate-documents", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "status", candidateId, requestId, status }) });
    const json = await response.json();
  ...
  async function sendMessage(mode: "selected" | "all" | "missing_documents") {
    ...
    const response = await fetch("/api/training/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "candidate_bulk", cohortId, candidateIds: ids, template: mode === "missing_documents" ? "missing_documents" : "custom", messageType: messageForm.type, subject: messageForm.subject, body: messageForm.body }) });
    const json = await response.json();
  ...
  async function downloadCohortPdf(type: "participants" | "attendance") {
    const response = await fetch("/api/training/cohort-documents", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ cohortId, type }) });
