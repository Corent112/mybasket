...
export default function TeamSelfEvaluationsTab({teamId}:{teamId:string}){
 ...
 const filtered=rows.filter(r=>{
   ...
   return d>=new Date(now.getFullYear(),7,1) || d<=new Date(now.getFullYear(),6,31);
 });
 ...
 return <section className="aeTab">
   ...
   <div className="panel"><h3>Évolution des séances</h3>{filtered.length===0?<div className="empty">Le visuel reste disponible. Il se remplira dès la première auto-évaluation.</div>:<div className="chart">{[...filtered].reverse().slice(-8).map((r:any)=><div className="barRow" key={r.id}><span>{new Date(r.review_date+"T12:00:00").toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit"})}</span><div className="bar"><i style={{width:`${score(r)*20}%`}}/></div><b>{score(r).toFixed(1)}</b></div>)}</div>}
