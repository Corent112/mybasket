...
import {attachPrivateSystemToPlaybook,createDrawingSeason,detachPrivateSystemFromPlaybook,duplicatePrivateSystem,ensureDrawingSeasons,listDrawingTeams,loadDrawingSystems,openPrivateSystemInDrawing,renamePrivateSystem,renameSeries,setPrivateSystemTags,type DrawingSeason,type DrawingSystem,type DrawingTeam} from '@/lib/drawing-workspace';
...
export default function ContentNavigator({embedded=false}:Props){
 ...
 const[view,setView]=useState<'playbook'|'library'>('playbook'),[expanded,setExpanded]=useState(false),[dragTarget,setDragTarget]=useState(''),[toast,setToast]=useState('');
 ...
 return <aside className={`content-nav-v46 ${embedded?'embedded':''} ${busy?'busy':''} ${expanded?'expanded':''}`}>
  ...
  {view==='library'?<>
    ...
    <div className='cn44-list-title'><b>Mes systèmes privés</b><span>{libraryVisible.length}</span></div><div className='cn44-list'>{libraryVisible.map(s=>systemRow(s))}</div>
    ...
    {selected&&<div className='cn46-nav-actions'>{selectedIndex>0&&<button onClick={()=>openPrivateSystemInDrawing(visible[selectedIndex-1].id)}>← Précédent</button>}<button onClick={()=>openPrivateSystemInDrawing(selected)}>Ouvrir</button>{selectedIndex>=0&&selectedIndex<visible.length-1&&<button onClick={()=>openPrivateSystemInDrawing(visible[selectedIndex+1].id)}>Suivant →</button>}<button onClick={()=>void doDuplicate(selected)}><Copy size={12}/> Dupliquer</button></div>}
   </>}
