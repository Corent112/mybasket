...
import {createPlaybookSeries,createPersonalSystemTag,deletePersonalSystemTag,renamePersonalSystemTag,ensureDefaultPersonalSystemTags,listPersonalSystemTags,type PersonalSystemTag,type PlaybookSeries} from '@/lib/playbook-series';
import {attachPrivateSystemToPlaybook,createDrawingSeason,detachPrivateSystemFromPlaybook,deletePrivateSystem,duplicatePrivateSystem,ensureDrawingSeasons,listDrawingTeams,loadDrawingSystems,renamePrivateSystem,renameSeries,setPrivateSystemTags,type DrawingSeason,type DrawingSystem,type DrawingTeam} from '@/lib/drawing-workspace';
...
export default function ContentNavigator({embedded=false,onPreviewSystem}:Props){
 ...
 async function submitCreate(){const name=createName.trim();if(!name)return;setBusy(true);try{
   ...
   if(modal==='playbook'){if(!teamId)throw new Error('Choisis une équipe : un Playbook doit obligatoirement être rattaché à une équipe.');const pb=await createPlaybook({title:name,team_id:teamId,season:season||null});await load();setPlaybookId(pb.id);setView('playbook');flash('Playbook créé dans Mes Playbooks')}
 ...
 async function deleteSystem(s:DrawingSystem){if(!confirm(`Supprimer définitivement « ${s.title} » ?`))return;setBusy(true);try{await deletePrivateSystem(s);await load();if(selected===s.id)setSelected('');setActionId('');flash('Système supprimé')}catch(e:any){alert(e?.message||'Suppression impossible')}finally{setBusy(false)}}
 ...
 async function renameFolder(name:string){const next=prompt('Nouveau nom du dossier ?',name);if(!next?.trim()||next.trim()===name)return;setBusy(true);try{await renamePersonalSystemTag(name,next.trim());await load();setTags(await listPersonalSystemTags());flash('Dossier renommé')}catch(e:any){alert(e?.message||'Renommage impossible')}finally{setBusy(false)}}
 async function deleteFolder(name:string){if(!confirm(`Supprimer le dossier « ${name} » ? Les systèmes resteront dans Mes systèmes.`))return;setBusy(true);try{await deletePersonalSystemTag(name);await load();setTags(await listPersonalSystemTags());flash('Dossier supprimé')}catch(e:any){alert(e?.message||'Suppression impossible')}finally{setBusy(false)}}
 function systemRow(s:DrawingSystem,compact=false){const opened=actionId===s.id;return <div key={s.id} className='cn-system-wrap'><article className={`cn-system ${selected===s.id?'selected':''} ${compact?'compact':''}`} draggable onDragStart={e=>{e.dataTransfer.setData('text/mybasket-system',s.id);e.dataTransfer.effectAllowed='move'}} onDragEnd={()=>setDragTarget('')} onClick={()=>previewSystem(s.id)}><div className='thumb'>{s.schemaImage?<img src={s.schemaImage} alt=''/>:'🏀'}</div><div className='meta'><b>{s.title}</b><small>{uniq([...(s.tags||[]),...s.inheritedTags]).slice(0,3).join(' · ')||'Non classé'}</small></div><button className='more' onClick={e=>{e.stopPropagation();setActionId(opened?'':s.id)}}><MoreHorizontal size={14}/></button></article>{opened&&<div className='cn44-menu'><button onClick={()=>previewSystem(s.id)}>Afficher sur le terrain</button><button className='danger' onClick={()=>void deleteSystem(s)}>Supprimer</button><button onClick={()=>void doDuplicate(s.id)}>Dupliquer et modifier</button><button onClick={()=>void renameSystem(s)}>Renommer</button><button onClick={()=>void editSystemTags(s)}>Modifier les tags</button>{view==='library'&&<button onClick={()=>openAddToPlaybook(s.id)}>+ Ajouter à un Playbook</button>}{playbookId&&s.playbookIds.includes(playbookId)&&<button className='danger' onClick={()=>void removeFromPlaybook(s)}>Retirer du Playbook</button>}</div>}</div>}
 return <aside className={`content-nav-v46 ${embedded?'embedded':''} ${busy?'busy':''} ${expanded?'expanded':''}`}>
  ...
  {view==='library'?<>
    ...
    <div className='cn50-folder-list'>{libraryFolders.length?libraryFolders.map(([folder,items])=>{const isOpen=openLibraryFolders.has(folder)||!!libraryQ;const label=folder==='__my_systems__'?'Mes systèmes':folder==='__unclassified__'?'Non classés':folder;return <div className='cn50-folder' key={folder}><div className='cn50-folder-head-row'><button className='cn50-folder-head' onClick={()=>toggleLibraryFolder(folder)}>{isOpen?<ChevronDown size={14}/>:<ChevronRight size={14}/>}<FolderOpen size={15}/><b>{label}</b><span>{items.length}</span></button>{folder!=='__my_systems__'&&folder!=='__unclassified__'&&<div className='cn50-folder-actions'><button title='Renommer le dossier' onClick={()=>void renameFolder(folder)}>✎</button><button title='Supprimer le dossier' onClick={()=>void deleteFolder(folder)}>🗑</button></div>}</div>{isOpen&&<div className='cn50-folder-children'>{items.map(s=>systemRow(s,true))}</div>}</div>}):<div className='cn50-empty'>Aucun système dans cette bibliothèque.</div>}</div>
