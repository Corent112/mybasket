...
export default function CreneauxPlanner({ clubId: forcedClubId }: { clubId?: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [gymRows, slotRows, availabilityRows] = await Promise.all([
        loadGymnases(id),
        loadSlots(id),
...
.color-dot.active {
  ...
  outline-offset: 3px;
      ...
      .slot-card strong, .slot-card span, .slot-card small { display: block; }
      .slot-card strong {
        width: 100%;
        font-size: 0.82rem;
        line-height: 1.15;
      ...
      .slot-meta { margin-top: 3px; color: var(--muted); font-size: 0.68rem; font-weight: 900; text-align: center; max-width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
      .slot-coach { margin-top: 2px; color: var(--muted); font-size: 0.66rem; font-weight: 800; text-align: center; max-width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
      .slot-card span {
        margin-top: 4px;
        font-size: 0.72rem;
        color: var(--text);
