...
export type PlaybookCategory = "Système demi-terrain" | "SLOB" | "BLOB";
...
function describeError(error: unknown, fallback: string): Error {
  if (error instanceof Error) {
    if (error.message === "Load failed" || error.message.includes("Failed to fetch")) {
      return new Error("Connexion à MyBasket impossible. Vérifiez votre connexion puis réessayez.");
    }
    return error;
  }
...
async function runPlaybookQuery<T>(
  action: () => PromiseLike<{ data: T; error: unknown }>,
  fallback: string
): Promise<T> {
  try {
    ...
    throw describeError(error, fallback);
  }
}
...
async function getUserId(): Promise<string> {
  ...
  try {
    ...
    console.warn("PLAYBOOK_AUTH_UNAVAILABLE", {
      session: sessionError?.message || null,
      user: userError?.message || null,
    });

    throw new Error(
      "Votre session n’est plus disponible. Reconnectez-vous puis réessayez."
    );
  } catch (error) {
    throw describeError(
      error,
      "Impossible de vérifier votre session. Reconnectez-vous puis réessayez."
    );
  }
}
...
export async function listPlaybooks(): Promise<Playbook[]> {
  ...
  if (error) throw describeError(error, "Erreur Playbook");
