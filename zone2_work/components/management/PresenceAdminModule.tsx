    ...
    <div className="presenceAdmin">
          ...
          <table>
            ...
            <tbody>
              {selectedTeam.players.map((player: any) => (
                <tr key={player.id}>
                  <td className="player">
                    <span className="avatar">{playerDisplayName(player).split(" ").map((part: string) => part[0]).join("").slice(0, 2).toUpperCase()}</span>
                    <span className="playerIdentity">
                      <strong>{playerDisplayName(player)}</strong>
                      <small>{playerPosition(player) || "Poste non renseigné"}</small>
                    </span>
                  </td>
      ...
      <style jsx>{`
        ...
        .player {
          ...
          align-items: center;
          gap: 10px;
          box-shadow: 8px 0 18px rgba(17,17,17,.04);
        }
        ...
        .avatar {
          ...
          background: linear-gradient(135deg,#6b1a2c,#3f0f19);
          color: #fff;
          font-size: 12px;
          font-weight: 950;
        }

        .playerIdentity { display:flex; flex-direction:column; min-width:0; }
        .playerIdentity strong { color:#6b1a2c; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        .playerIdentity small { font-size:12px; color:#777; margin-top:2px; }
