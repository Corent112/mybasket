...
const defaultBlock = (title = "Équipes de 3", playersPerTeam = 3): TeamCompositionBlock => ({ id: uid("block"), title, playersPerTeam, teams: [1, 2, 3].map((n) => ({ id: uid("team"), name: `Équipe ${n}`, playerIds: [] as string[] })) });
...
function legacyBlocks(groups?: Record<string, string[]> | null): TeamCompositionBlock[] {
  const teams: CompositionTeam[] = Object.entries(groups ?? {}).map(
    ([name, playerIds]): CompositionTeam => ({
      id: uid("team"),
      name,
...
function recurrenceDates(startDate: string, type: "weekly" | "monthly" | "yearly", count: number) {
  const start = new Date(`${startDate}T12:00:00`);
  if (Number.isNaN(start.getTime())) return [] as string[];
  return Array.from({length: Math.min(Math.max(count || 1, 1), 120)}, (_, i) => {
    const d = new Date(start);
    if (type === "weekly") d.setDate(start.getDate() + i * 7);
    if (type === "monthly") { const day=start.getDate(); d.setMonth(start.getMonth()+i); if(d.getDate()!==day)d.setDate(0); }
...
export default function NouvelleSeancePage() {
  ...
  const [calendarCandidates, setCalendarCandidates] = useState<CalendarMatchCandidate[]>([]);
  const [calendarCandidateLoading, setCalendarCandidateLoading] = useState(false);
  const [selectedCalendarCandidate, setSelectedCalendarCandidate] = useState<CalendarMatchCandidate | null>(null);
  ...
  useEffect(() => {
    if (editingId || !teamId || !date) { setCalendarCandidates([]); setSelectedCalendarCandidate(null); return; }
    void findCalendarCandidates(teamId, date);
  }, [teamId, date, editingId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function load() {
    ...
    if (sessionId) {
      ...
      const directIds: string[] = (directRows ?? [])
        .filter((r: any) => r.selected !== false && !["absent", "injured", "excused"].includes(String(r.status || "")))
        .map((r: any) => String(r.player_id || r.id || ""))
        .filter((id: string) => Boolean(id));
      const attendanceIds: string[] = (attendanceRows ?? [])
        .filter((r: any) => !["absent", "injured", "excused"].includes(String(r.status || "present")))
        .map((r: any) => String(r.player_id || ""))
        .filter((id: string) => Boolean(id));
      setSelectedPlayers(Array.from(new Set<string>(directIds.length ? directIds : attendanceIds)));
    } else {
  ...
  async function findCalendarCandidates(targetTeamId: string, targetDate: string) {
    setCalendarCandidateLoading(true);
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) { setCalendarCandidates([]); setCalendarCandidateLoading(false); return; }

    const [{ data: realRows }, { data: recurringRows }] = await Promise.all([
      supabase.from("calendar_events")
        .select("id,title,event_date,start_time,end_time,location,team_id,recurrence_id")
        .eq("user_id", user.id).eq("team_id", targetTeamId).eq("event_type","training").eq("event_date",targetDate),
      supabase.from("calendar_recurrences")
        .select("id,title,start_date,start_time,end_time,location,team_id,recurrence_type,occurrence_count")
        .eq("user_id", user.id).eq("team_id", targetTeamId).eq("event_type","training").eq("is_active",true)
    ]);

    const real: CalendarMatchCandidate[] = (realRows ?? []).map((r:any) => ({
      id:String(r.id), source:"event", title:String(r.title || "Entraînement"), event_date:targetDate,
      start_time:r.start_time ?? null, end_time:r.end_time ?? null, location:r.location ?? null,
      team_id:r.team_id ?? null, recurrence_id:r.recurrence_id ?? null
    }));

    const existingSeries = new Set(real.filter(r=>r.recurrence_id).map(r=>`${r.recurrence_id}|${targetDate}`));
    const virtual: CalendarMatchCandidate[] = [];
    for (const r of recurringRows ?? []) {
      if (!recurrenceDates(String(r.start_date), r.recurrence_type, Number(r.occurrence_count||1)).includes(targetDate)) continue;
      if (existingSeries.has(`${r.id}|${targetDate}`)) continue;
      virtual.push({
        id:`virtual:${r.id}:${targetDate}`, source:"recurrence", title:String(r.title || "Entraînement"),
        event_date:targetDate, start_time:r.start_time ?? null, end_time:r.end_time ?? null,
        location:r.location ?? null, team_id:r.team_id ?? null, recurrence_id:String(r.id)
      });
    }

    const all = [...real, ...virtual].sort((a,b)=>shortTime(a.start_time).localeCompare(shortTime(b.start_time)));
    setCalendarCandidates(all);
    setCalendarCandidateLoading(false);
    if (all.length === 1) applyCalendarCandidate(all[0]); else setSelectedCalendarCandidate(null);
  }

  function applyCalendarCandidate(candidate: CalendarMatchCandidate) {
    setSelectedCalendarCandidate(candidate);
    if (candidate.start_time) setStartTime(shortTime(candidate.start_time));
    if (candidate.end_time) setEndTime(shortTime(candidate.end_time));
  ...
  function addTeam(blockId: string) {
    setBlocks((prev) =>
      prev.map((block) =>
        block.id === blockId
          ? {
  ...
  function autoDistribute(blockId: string) {
    setBlocks((prev) => prev.map((block) => {
      ...
      const teamsCopy: CompositionTeam[] = [...block.teams];
      ...
      const cleared: CompositionTeam[] = teamsCopy.map((team) => ({
        ...team,
        playerIds: [] as string[],
      }));
      const teamCount = Math.min(needed, cleared.length);
      selectedPlayers.forEach((playerId, index) => {
        const targetTeam = cleared[index % teamCount];
  ...
  async function save() {
    ...
    const calendar = {
      user_id:user.id, visibility:"private", event_type:"training", session_id:sessionId,
      team_id:teamId || null,
      title:`${selectedTeam?.name ?? "Équipe"} • ${theme.trim()}`,
      description:`${title.trim() || "Séance rapide"} — Ouvrir la fiche séance`,
      event_date:date, start_time:startTime || null, end_time:endTime || null,
      location:location || null, attachment_url:null
    };

    const { data: linked } = await supabase.from("calendar_events").select("id").eq("session_id", sessionId).maybeSingle();
    if (linked?.id) {
      await supabase.from("calendar_events").update(calendar).eq("id", linked.id);
    } else {
      const { data: sameDay } = await supabase.from("calendar_events")
        .select("id,start_time").eq("user_id",user.id).eq("team_id",teamId)
        .eq("event_type","training").eq("event_date",date).is("session_id",null);
      const exact = (sameDay ?? []).find((r:any)=>shortTime(r.start_time)===shortTime(startTime));

      if (exact?.id) {
        await supabase.from("calendar_events").update(calendar).eq("id",exact.id);
      } else if (
        selectedCalendarCandidate?.source === "recurrence" &&
        selectedCalendarCandidate.recurrence_id &&
        selectedCalendarCandidate.event_date === date &&
        shortTime(selectedCalendarCandidate.start_time) === shortTime(startTime)
      ) {
        await supabase.from("calendar_events").upsert({
          ...calendar,
          recurrence_id:selectedCalendarCandidate.recurrence_id,
  ...
  return <main className="page">
    ...
    {!editingId && teamId && date && <section className="calendarLinkPanel">
      <div className="calendarLinkHead"><div><span>CALENDRIER</span><strong>{calendarCandidateLoading ? "Recherche…" : calendarCandidates.length ? `${calendarCandidates.length} créneau${calendarCandidates.length>1?"x":""} trouvé${calendarCandidates.length>1?"s":""}` : "Aucun créneau existant"}</strong></div>{selectedCalendarCandidate && <b>✓ Prérempli</b>}</div>
      {calendarCandidates.length > 0 ? <div className="calendarCandidates">{calendarCandidates.map(candidate => <button type="button" key={candidate.id} className={selectedCalendarCandidate?.id===candidate.id ? "calendarCandidate active":"calendarCandidate"} onClick={()=>applyCalendarCandidate(candidate)}><span className="candidateTime">{shortTime(candidate.start_time)||"—"}</span><span><strong>{candidate.title}</strong><small>{candidate.location || "Lieu non renseigné"}{candidate.source==="recurrence"?" · ↻ récurrent":""}</small></span><em>{selectedCalendarCandidate?.id===candidate.id?"Utilisé ✓":"Reprendre"}</em></button>)}</div> : !calendarCandidateLoading && <p className="calendarEmpty">La séance créera un nouvel événement calendrier.</p>}
      {selectedCalendarCandidate && <div className="calendarMergeHint"><strong>Fusion automatique si équipe + date + heure identiques.</strong><span>La fiche séance sera rattachée au créneau existant : aucun doublon dans le calendrier.</span></div>}
    </section>}
