...
import NextSessionAdvicePanel from "@/components/development/NextSessionAdvicePanel";
...
export default function NouvelleSeancePage() {
  ...
  async function save() {
    ...
    if (linkedEventsError) {
      console.error("Erreur synchronisation calendrier:", linkedEventsError);
      setSaving(false);
      return alert("La séance est enregistrée, mais le calendrier n'a pas pu être synchronisé.");
    }

    const linkedIds = (linkedEvents ?? [])
      .map((row: any) => String(row.id || ""))
      .filter(Boolean);

    if (linkedIds.length) {
      const [primaryId, ...duplicateIds] = linkedIds;
      ...
      if (updateError) {
        setSaving(false);
        return alert(`Séance enregistrée, mais calendrier non mis à jour : ${updateError.message}`);
      }
      ...
      const exact = (sameDay ?? []).find(
        (row: any) => shortTime(row.start_time) === shortTime(startTime),
      );
      ...
      if (exact?.id) {
        ...
        if (insertCalendarError) {
          setSaving(false);
          return alert(`Séance enregistrée, mais calendrier non créé : ${insertCalendarError.message}`);
        }
      }
    }
  ...
  return <main className="page">
    ...
    {teamId && <NextSessionAdvicePanel teamId={teamId} />}
