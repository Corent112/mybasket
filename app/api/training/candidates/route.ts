...
export async function POST(request:Request){
 ...
 if(!uid){const site=(process.env.NEXT_PUBLIC_SITE_URL||"https://mybasket.vercel.app").replace(/\/$/,"");const q=await admin.auth.admin.inviteUserByEmail(email,{redirectTo:`${site}/auth/callback?next=${encodeURIComponent("/formation/mes-formations")}`});if(q.error)return NextResponse.json({error:q.error.message},{status:400});uid=q.data.user?.id||""}
 ...
 const q=await admin.from("training_candidates").upsert({cohort_id:cohortId,user_id:uid,status:"active",progression:0},{onConflict:"cohort_id,user_id"});
