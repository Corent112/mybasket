...
export async function POST(req: Request) {
  ...
  const { data: cohort, error: cohortError } = await admin
    ...
    .select(
      "id,name,institution_id,planning_title,start_date,end_date,location",
    )
    .eq("id", cohortId)
    .maybeSingle();

  if (cohortError || !cohort?.institution_id) {
    return NextResponse.json(
      { error: cohortError?.message || "Promotion introuvable" },
      { status: 404 },
    );
  }
  ...
  const [
    ...
    admin
      ...
      .select(
        "id,name,short_name,logo_url,email,city,document_primary_color,document_secondary_color",
      )
    ...
    admin
      ...
      .select(
        "id,training_day,start_time,end_time,title,formation_name,instructor_name,room_name,location_type,block_type,description",
      )
  ...
  const buffer = await renderToBuffer(
    React.createElement(TrainingPlanningPdf, {
      structure,
      cohort,
      title,
      blocks: blocks || [],
