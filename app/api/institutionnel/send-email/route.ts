...
export async function POST(req:Request){
 ...
 const result=await sendTransactionalEmail({to,subject,html:`<div style="font-family:Arial,sans-serif;max-width:760px;margin:auto"><div style="background:#6B1A2C;color:white;padding:18px 22px"><strong>MyBasket Institutionnel</strong></div><div style="padding:22px;white-space:pre-wrap">${esc(text)}</div></div>`});
