...
export async function POST(req:Request){
 ...
 const body=await req.json().catch(()=>({}));const structureId=String(body.structureId||"");const to=Array.isArray(body.to)?body.to.map(String).filter(Boolean):[];const subject=String(body.subject||"").trim();const text=String(body.body||"").trim();const eventId=body.eventId?String(body.eventId):null;const selectionId=body.selectionId?String(body.selectionId):null;
 ...
 await db.from("institutional_communications").insert({structure_id:structureId,sender_user_id:user.id,subject,body:text,recipient_emails:to,status:result.sent?"sent":"failed",provider_id:result.sent?result.providerId:null,event_id:eventId,selection_id:selectionId});
