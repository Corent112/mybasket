...
export async function POST(req:Request,{params}:{params:Promise<{token:string}>}){try{
  ...
  const body=await req.json().catch(()=>({}));const targetStructureId=String(body.structureId||"");const{data:member}=await db.from("institutional_members").select("id").eq("structure_id",targetStructureId).eq("user_id",user.id).eq("status","active").maybeSingle();if(!member)return NextResponse.json({error:"Choisissez une institution à laquelle vous appartenez."},{status:403});
  ...
  const u=await db.from("institutional_player_transfers").update({target_structure_id:targetStructureId,status:"accepted",accepted_by:user.id,accepted_at:new Date().toISOString()}).eq("id",t.id);if(u.error)return NextResponse.json({error:u.error.message},{status:400});return NextResponse.json({ok:true,count:shares.length});
