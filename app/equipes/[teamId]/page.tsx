...
type TeamMainTab = "presentation" | "stats";
...
export default function EquipeDetailPage({
  ...
  async function changeBanner(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    saveTeam({ ...team, banniere });
    reload();
    flash("Photo d'équipe mise à jour ✓");
  }

  function handleSaveTeam(t: Team) {
    saveTeam(t);
  ...
  function handleSavePlayer(p: Player) {
    upsertPlayer(teamId, p);
    setPlayerForm({ open: false });
    reload();
    flash(p.id ? "Joueur enregistré ✓" : "Joueur ajouté ✓");
  }

  function handleDelete(p: Player, e: React.MouseEvent) {
    e.stopPropagation();
    if (confirm(`Retirer ${p.firstName} ${p.lastName} de l'effectif ?`)) {
      deletePlayer(teamId, p.id);
      reload();
      flash("Joueur retiré");
    }
  }
