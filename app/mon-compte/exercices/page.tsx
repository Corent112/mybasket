...
export default function MesExercicesPage(){
 ...
 const counts=useMemo(()=>({all:items.length,draft:items.filter(x=>status(x)==="draft").length,submitted:items.filter(x=>status(x)==="submitted").length,approved:items.filter(x=>status(x)==="approved").length,rejected:items.filter(x=>status(x)==="rejected").length,favorites:items.filter(x=>favoriteIds.has(x.id)).length}),[items,favoriteIds]);
