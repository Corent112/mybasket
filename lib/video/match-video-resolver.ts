...
import { rememberSessionVideos } from "@/lib/video/video-library";
...
export async function attachMatchVideoFile(
  ...
  // Navigateurs sans handle persistant (Safari, Firefox) : la vidéo choisie
  // alimente la bibliothèque de session, donc les autres projets, la fiche
  // équipe et la fiche individuelle la retrouvent sans nouvelle sélection.
  rememberSessionVideos([file]);
...
export async function relinkMatchVideo(
  ...
  // Idem : une seule sélection manuelle suffit pour toute la session.
  rememberSessionVideos([picked.file]);
